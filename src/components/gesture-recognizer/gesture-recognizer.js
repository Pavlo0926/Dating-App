...
class GestureRecognizer extends Component {
  ...
  _handlePanResponderMove(evt, gestureState) {
    if (gestureState.dy < 0 && this.props.enableMoveUp) {
      Animated.event([null, { dy: this.pan.y }], { useNativeDriver: false })(
        evt,
        gestureState,
      );
    } else if (gestureState.dy > 0 && this.props.enableMoveDown) {
      Animated.event([null, { dy: this.pan.y }], { useNativeDriver: false })(
        evt,
        gestureState,
      );
  ...
  _triggerSwipeHandlers(swipeDirection, gestureState) {
    ...
    switch (swipeDirection) {
      ...
      case SWIPE_RIGHT:
        ...
        break;
      case SWIPE_UP:
        if (this.props.enableMoveUp && this.props.maxMoveUp) {
          Animated.timing(this.pan, {
            toValue: { x: 0, y: -this.props.maxMoveUp },
            duration: 100,
        ...
        break;
      case SWIPE_DOWN:
        if (this.props.enableMoveDown && this.props.maxMoveDown) {
          Animated.timing(this.pan, {
            toValue: { x: 0, y: this.props.maxMoveDown },
            duration: 100,
            useNativeDriver: false,
          }).start(() => {
      ...
      default:
        ...
        } else if (this.props.enableMoveDown && this.props.maxMoveDown) {
          if (gestureState.dy > 30) {
            Animated.timing(this.pan, {
              toValue: { x: 0, y: this.props.maxMoveDown },
              duration: 100,
