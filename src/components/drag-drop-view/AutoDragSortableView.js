...
export default class AutoDragSortableView extends Component {
  constructor(props) {
    ...
    const itemWidth =
      props.childrenWidth + props.marginChildrenLeft + props.marginChildrenRight;
    const itemHeight =
      props.childrenHeight + props.marginChildrenTop + props.marginChildrenBottom;

    // this.reComplexDataSource(true,props) // react < 16.3
    // react > 16.3 Fiber
    const rowNum = parseInt(props.parentWidth / itemWidth, 10);
    const dataSource = props.dataSource.map((item, index) => {
      const newData = {};
      const left = (index % rowNum) * itemWidth;
      const top = parseInt(index / rowNum, 10) * itemHeight;
      ...
      newData.position = new Animated.ValueXY({
        x: parseInt(left + 0.5, 10),
        y: parseInt(top + 0.5, 10),
      });
  ...
  static getDerivedStateFromProps(nextprops, prevState) {
    ...
    if (
      ...
      const rowNum = parseInt(nextprops.parentWidth / itemWidth, 10);
  ...
  dealtScrollStatus = () => {
    ...
    if (horizontal) {
      if (horizontal && (scrollData == null || scrollData.offsetX == null)) return;
      const { totalWidth, windowWidth, offsetX } = scrollData;
      if (totalWidth <= windowWidth + offsetX) {
    ...
    this.autoInterval = setInterval(() => {
      ...
      if (this.props.horizontal) {
        if (this.autoObj.forceScrollStatus === 1) {
          this.autoObj.scrollDx = this.autoObj.scrollDx + this.props.autoThrottle;
        } else if (this.autoObj.forceScrollStatus === -1) {
          this.autoObj.scrollDx = this.autoObj.scrollDx - this.props.autoThrottle;
        }
        this.scrollTo(this.autoObj.scrollDx, false);
        ...
        if (Platform.OS === "android") {
          setTimeout(() => {
            if (this.isHasMove)
              this.moveTouch(null, {
                dx: this.autoObj.curDx + this.autoObj.scrollDx,
  ...
  startTouch(touchIndex) {
    ...
    if (this.sortRefs.has(touchIndex)) {
      ...
      if (this.isStartupAuto()) {
        if (this.props.horizontal) {
          this.autoObj.scrollDx = this.autoObj.hasScrollDx = this.curScrollData.offsetX;
        } else {
  ...
  moveTouch(nativeEvent, gestureState) {
    ...
    if (this.touchCurItem) {
      ...
      const rowNum = parseInt(this.props.parentWidth / itemWidth, 10);
      const maxWidth = this.props.parentWidth - itemWidth;
      const maxHeight =
        itemHeight * Math.ceil(this.state.dataSource.length / rowNum) - itemHeight;
      ...
      if (this.isStartupAuto()) {
        if (this.props.horizontal) {
          const curDis = this.touchCurItem.originLeft + dx - this.autoObj.hasScrollDx;
          if (nativeEvent != null) {
            const tempStatus = this.autoObj.forceScrollStatus;
            // Automatic sliding
            const minDownDiss =
              curDis +
              this.props.childrenWidth * (1 + (this.props.maxScale - 1) / 2) +
              this.props.marginChildrenLeft;
            const maxUpDiss = curDis + this.props.marginChildrenLeft;
            if (
              (tempStatus === 0 || tempStatus === 2) &&
              vx > 0.01 &&
              minDownDiss > this.curScrollData.windowWidth
            ) {
              this.autoObj.curDx = dx;
          ...
          if (vx != null) {
            // Slide down 1、2
            if (this.autoObj.forceScrollStatus >= 1 && -vx > 0.01) {
              this.autoObj.forceScrollStatus = 0;
              // Slide up -1、-2
            } else if (this.autoObj.forceScrollStatus <= -1 && vx > 0.01) {
              this.autoObj.forceScrollStatus = 0;
            }
          }

          // Remember the X axis
          this.autoObj.scrollDy = dy;
          // Correction data 1
          dx = dx - this.autoObj.hasScrollDx;
          if (nativeEvent != null) {
            // Correction data 2
            dx = dx + this.autoObj.scrollDx;
      ...
      if (moveXNum > 0) {
        moveXNum = parseInt(moveXNum + 0.5, 10);
      } else if (moveXNum < 0) {
        moveXNum = parseInt(moveXNum - 0.5, 10);
      }
      if (moveYNum > 0) {
        moveYNum = parseInt(moveYNum + 0.5, 10);
      } else if (moveYNum < 0) {
        moveYNum = parseInt(moveYNum - 0.5, 10);
      }
        ...
        this.state.dataSource.forEach((item, index) => {
          ...
          if (nextItem != null) {
            Animated.timing(item.position, {
              toValue: {
                x: parseInt(nextItem.originLeft + 0.5, 10),
                y: parseInt(nextItem.originTop + 0.5, 10),
              },
    ...
    if (startIndex === endIndex) {
      ...
      if (curItem !== null) {
        curItem.position.setValue({
          x: parseInt(curItem.originLeft + 0.5, 10),
          y: parseInt(curItem.originTop + 0.5, 10),
        });
      }
      return;
    }
    ...
    const newDataSource = [...this.state.dataSource].map((item, index) => {
      ...
      if (newIndex != null) {
        ...
        newItem.position = new Animated.ValueXY({
          x: parseInt(item.originLeft + 0.5, 10),
          y: parseInt(item.originTop + 0.5, 10),
        });
        item = newItem;
      }
    ...
    this.setState(
      ...
      () => {
        ...
        this.state.dataSource[startIndex].position.setValue({
          x: parseInt(startItem.originLeft + 0.5, 10),
          y: parseInt(startItem.originTop + 0.5, 10),
        });
        const endItem = this.state.dataSource[endIndex];
        this.state.dataSource[endIndex].position.setValue({
          x: parseInt(endItem.originLeft + 0.5, 10),
          y: parseInt(endItem.originTop + 0.5, 10),
        });
      },
    );
  }
  ...
  scrollTo = (height, animated = true) => {
    ...
    if (this.curScrollData) {
      if (this.props.horizontal) {
        if (this.autoObj.forceScrollStatus < 0 && this.curScrollData.offsetX <= 0) {
          this.autoObj.scrollDx = 0; // Correcting data system deviations
          return;
        } else if (
          this.autoObj.forceScrollStatus > 0 &&
          this.curScrollData.windowWidth + this.curScrollData.offsetX >=
            this.curScrollData.totalWidth
        ) {
          this.autoObj.scrollDx = this.curScrollData.offsetX; //Correcting data system deviations
          return;
        }
      } else {
    ...
    if (this.scrollRef !== null) {
      if (this.props.horizontal) {
        this.scrollRef.scrollTo({ x: height, y: 0, animated });
      } else {
  ...
  onScrollListener = ({ nativeEvent }) => {
    this.curScrollData = {
      ...
      hasScroll: true,
      totalWidth: nativeEvent.contentSize.width,
      windowWidth: nativeEvent.layoutMeasurement.width,
